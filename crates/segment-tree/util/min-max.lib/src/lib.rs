...
use max_monoid::max_monoid;
use min_exists::MinExists;
use segment_tree::{segment_tree_by_monoid, SegmentTree};
use segment_tree_util_type::seg_type;
use std::cmp;

pub fn segment_tree_new_max<T>(vec: Vec<T>) -> seg_type!(T = T)
where
    T: Clone + PartialOrd + Ord + MinExists,
{
    segment_tree_by_monoid(vec, max_monoid().into())
}
